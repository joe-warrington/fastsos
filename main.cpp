...
int main(int argc, char * argv[]) {
    ...
    if (argc == 2 || argc == 3 || argc == 4) { // Input validation
        d = stoi(argv[1]);
        if (d < 0) {
        ...
        if (argc == 4) {
            ...
            if (solver_choice != "mosek" && solver_choice != "scs" && solver_choice != "sosadmm") {
                cout << "  \033[1;31m" << "Cannot use solver '" << positivity_condition <<
                     "'. Can only use MOSEK, SCS, or SOSADMM." << "\033[0m" << endl;
                return 1;
            }
        }
    }
...
//    auto problem_tuple = read_problem_from_file("../input.txt");
    auto problem_tuple = read_problem_from_file("../problems/Bex2_1_3.txt");
